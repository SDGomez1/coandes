...
export default function EditCustomer({ customer }: EditCustomerProps) {
  ...
  const editCustomer = useMutation(api.customers.editCustomer);
    ...
    try {
      ...
      toast.error(
        (error instanceof Error ? error.message : undefined) ??
          "Tenemos problemas para eliminar el cliente, inténtalo más tarde",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      console.error(error);
      toast.error(
        "Tuvimos problemas para actualizar el cliente, inténtalo más tarde",
      );
    } finally {
      setIsLoading(false);
    }
